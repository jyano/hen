...
//start over with geo (fn by fn) !!!

M.poly=function(){//gpcas
    var g=G(arguments), p,vs
    p= new PolyDefault()

    if(b2d.isFixt(g[0])){ vs = g[0].wV() }
    else if(b2d.isBody(g[0])){ vs = g[0].f().wV()}
    else {vs= g[1]? g :  g[0]}
    p.addPoints(vs)
    return p}
...
f.rV=    f.v =  f.vs=  f.verts=function(){var f=this, b=f.B(), g=G(arguments),
// local verts rotated locally by body's rotation
    vs=_.m( f.H().m_vertices, b2d.mult )
    if(g.p){vs = _.m(vs, function(v){ return v.rot(b.rot()) })}
    return vs
}

f.wV =f.V=f.wVerts=function(){
    var f=this, b=f.B(), g=G(arguments),vs     //wor vs of f rotated (or optionally, not rotated for some reason)
    vs = g.N? f.vs('+') : f.vs()
    return _.m(vs, function(v){return v.add(b)})
}
...
GEOF=function(){W()
    ...
    //goal: subtract b from r



    r = w.S(600,300, 'r', 400,200).rot(20).f()
    b = w.S(600,300, 'b', 400,200).rot(-20).f()
...
b2d.vs=  b2d.verts=function(){var g=G(arguments)
...
b2d.polyDot = function (vs) {// see f.dots
    var t = 0
    _.e(
        A(vs) ? vs : vs.vs(), //combine with b2d.verts
        function (v) {
            setTimeout(function () {w.dot(V(v))}, t)
            t += 100 }
    )
}
...
b2d.sep =  b2d.separator = function(body, verts, scale){//b2d.fig= b2d.conc =
...
//////////////////////////////////////////////////////////////////////////////////////////
f.tPol=f.poly= f.polyVerts=function(){
    var f=this, p
    p=M.poly(f)
    return p}
b.rel = function (){var b = this,g=G(arguments)
    ...
    return _.m(
        b2d.isGPoly(g[0]) ? g[0].vs() : g[0],
        function (v) {
            return V(  v[0]-b.X(), v[1]-b.Y() ) })
}
f.minus = f.diff = function () {var f=this, b=f.B(),g=G(arguments),poly //can handle a fixt OR a body! or even a gPoly itself!
    poly = M.poly(f).difference( pol(g[0]) ); _.e(_.r(g), function(p){poly = poly.difference(pol(p)) })
    if (b2d.hasVerts(poly)) {

        return b.rel(poly)

    }  else {alert('does not have verts! its completely vertless!')}

    function pol(a){return b2d.isGPoly(a) ? a : M.poly(a)} }
f.sub=f.DIF=f.DIFF=function(b2){var f=this,b= f.B(), g = G(arguments)
...
f.union = function (f2) {var f = this, b = f.B(), p //can handle a fixt OR a body!
        if (A(f2)) {  return f.union.apply(f, f2)  }

    p = f.tPol().union(  f2.polyVerts()  )

        _.e( _.r(arguments), function (f) {  p = p.union(f.tPol())  })
        return  b.rel(p)
    }
f.area = function () {var f=this
        return M.poly(f.V()).getArea() }
b2d.newX=newX = function (x, y, r) {r = M.toRadians(r)
    return x * M.cos(r) - y * M.sin(r)}
b2d.newY=newY = function (x, y, r) {r = M.toRadians(r)
    return x * M.sin(r) + y * M.cos(r)}
b.vs=  b.verts=function(){var b=this, n,fs, p
//return this.union()
        //it used to just get from the FIRST fixt,
        //but now it unions them all together :)
        //b.verts = function(){ return this.fixt().verts() }
    n = b.num(); if (n == 0) {return}
    fs = b.fs()
    p = fs[0]

    if(n==1){return p; return p.union(p)  }

    _.e(_.r(fs), function (f){
        p = M.poly(p.union(M.poly(f)))
    })
    return p

}
b.rotVerts = function () {var    b = this, vs//rotated but local
    vs = b.vs()
...
b.wV= b.V=   b.rotWorVerts=function(){
    var b=this

    return _.m(b.rotVerts(),
...
b.wVerts = function(){var b=this,vs
   vs = _.m( b.vs(),
        function(v){ return v.add( b.transform().position.mult() )  })
return vs}
...
b.dff = function (b2) {var b=this
    return M.poly( b.V() )
        .difference(M.poly( b2.V() ))}
b.DIFF = function (b2) {
    var b = this, g=G(arguments), f=b.f(),

    b2 = g[0]

    b2d.sep(
        b,
        _.m(   b.dff(b2).vs(),    function (v) {  return V(v[0] - b.X(), v[1] - b.Y())  } )
    )
...
b.DIF = function (b2) {var g = G(arguments),b=this,

    b2 = g[0]

    b.fs(function(f){ f.DIFF(b2) })
...
b.wPolygon = function () {var b=this
        return M.poly(b.wVerts())}


b.polyVerts=function(){var b=this
...
b.minusPolyCirc= function (x, y, r, sides) {var b=this, poly, vs
    poly = M.poly(
        _.m(b2d.polyCirc(r, sides),
            function(v){return [v.x+x, v.y+y]}) )
    vs = b.rel(b.polyVerts().difference(poly))
    b.clear().sep(vs)
    return b}
...
b.minus = function () {var b=this, g=G(arguments)


    df = b.polyVerts().difference(
        g[0].polyVerts()
    )
...
BMIN=function(){W()
    ...
    b=w.S(300,300, 'b', 300,200)

    r= w.S(300, 380, 'r', 100,250)
...
b.sub= function (a){var b=this,g=G(arguments)
...
b.subtract = function () {var b=this, g = G(arguments)
    b.fs(function (f) { f.sub(g[0]) })
    if (g.n) { g[0].kill() }
...
b.polyClone = function (x, y, a) {var b = this,b2

    x = N(x) ? x : b.X()
    y = N(y) ? y : b.Y()
    a = N(a) ? a : b.rot()

    b2 = w.B(x, y).type(b.type()).rot(a)

    _.e(b.fs(), function (f){

        b2.poly.apply(
            b2,
            _.m(f.vs(), function(v){ return [v.x,v.y]  })
        )
...
DEST=function(){W({g:1}).Y()
        ...
        if(fixt=cx.with('bul','terr')){
            ...
            if(can){can=false
                setTimeout(function(){// br =  w.brick(bX,bY,60,60).rot(45)
                    br=b2d.sep(  b2d.polyCirc(20,7)    ).XY(bX,bY)
                    b.each(function(f){   f.DIFF(br)  })
                    br.kill()
                    can=true}, 10)
            }
        }
    ...
    function killIfSmall(f){var area=this.area()
...
f.uni=function(){var f=this, g=G(arguments),p1,p
    p1= M.poly(f)
    p=p1.union( M.poly(g[0]) )
    if(g.p){ b2d.polyDot(p) }
    return p}
f.dif=function(){var f=this, g=G(arguments),p1,p
    p1= M.poly(f)
    p=p1.difference( M.poly(g[0]) )
    if(g.p){ b2d.polyDot(p) }
    return p}
f.int=function(){var f=this, g=G(arguments),p1,p
    p1= M.poly(f)
    p=p1.intersection( M.poly(g[0]) )
    if(g.p){ b2d.polyDot(p) }
    return p}
f.xor=function(){var f=this, g=G(arguments),p1,p
    p1= M.poly(f)
    p=p1.xor( M.poly(g[0]) )
    if(g.p){ b2d.polyDot(p) }
    return p}
...
BODMINUS = function () {W(0)



    f = w.S(300, 400, 'w',  100,  100).f()
    ...
    p = M.poly(M.poly(f) ).difference( b.fs()[1]   )
...
REDUCE = function () { W()
    b = w.S(300, 400)
    f = b.D(100, 200)
    f1 = b.D(200, 100, 'b',100, 0)
    f2 = b.D(200, 100,'r', 100, 0, 36)
    f3 = b.D(20, 20, 'g', 200, 0)
    u =  f.union( [f1, f2] )
    u = b.union()
    b2 = w.D(600, 300).sep(u)
    // body.polyVerts() does an automatic union of ALL its verts! i think :)

}


UNIONNOTTOUCHING = function () {W()
    ...
    //p = M.poly( v )  //.union(b2.polyVerts())



    /*

    _.e(p.verts(), function (v) { w.dot(v[0], v[1])})
    ...
    _.e(p2.verts(), function (v) { w.dot('r', v[0], v[1])})
    ...
    _.e(p3.verts(), function (v) {  w.dot(v[0], v[1])  })
    ...
    _.e(p4.verts(),    function (v) { w.dot(v[0], v[1])  })
...
b.union= function () {var b=this, n, fs
    ...
    if (n == 0) {return}
    if (n == 1) {return fs[0].polyVerts()


    }

    //return fs[0].union(_.r(fs))
}
...
elegant=function() {
    ...
    DIF = function () { W()
        ...
        b4=w.D(300, 400, 'r', [-100, 10], [-80, -40], [0, -200], [100, 0])
    ...
    MPOLY=function(){W({g:10, w:'L'})
//[ //] //outer brackets are optional

        poly = M.poly( [300,300], [400,100], [600, 400]  )

        poly1 = M.poly([ [200,300],[400,150],[ 400, 450]  ])

        w.S(0,0).sep({c:'b', v:poly,  s:1})

        w.S(0,0).sep({c:'r', v:poly1,  s:1})

        w.D(0,0).sep({c:'y',  v: poly1.xor(poly)  })

        /*
         p2= p1.difference(p)
         d= w.S(150,50).sep({  c:'y',  v:p2.vs() })

         p3= p1.union(p)
         u= w.S(150,50).sep({  c:'y',  v:p3.vs() })

         p4= p1.intersection(p)
         i= w.S(150,50).sep({  c:'y',  v:p4.vs() })
         */
        $.in(2, function(){  //x.dyn()   //d.dyn(); i.dyn(); u.dyn()
        })

    }


    DEST1=function(){W({g:0})
        y= w.ship()
    ...
    POL=function(){W([1200,600,2400,600],{g:0}).C('z').Y()
        ...
        b=w.D(100,300)
        b.pol({s:1,   c:'y',  C:'r', l:10, v:[  [-200,-100],[0,-200],[100, -100]  ]})
        b.pol({  C: 'o',c:'b', l:10,v: [  [-100, 0],  [0, -200],  [100, 20],    [0, -150] ]})
        b2=w.D(200,300)
        b2.cir({
            k:'cir', r:100,   x:-20,
            d:.2, b:.8, f:100,
        ...
        b3 = w.D(800, 400)
        b3.pol('o', [-300, 0], [0, -100], [10, 150])
        b3.pol([  [-10, 0], [0, -100],  [100, -50] ])
        b3.pol('b', [ [-100, 0], [0, -100], [10, 150]  ])
        b3.pol('r',   [-100, 0], [0, -100],  [100, -50]  )

    }
    BODVERTS = function () { W(0).C('r').Y()

        var fs

        b = w.S(200, 200,'y', 100, 100)
        b.rec({c:'b', w:20, h:200})

        fs = b.fs()

        p = fs[0].uni(fs[1], '+') //<-- refactore from !!! ->  b2d.polyDot(   p = M.poly( fs[0] ).union(  M.poly( fs[1] )  ))

        w.D(400, 200).sep('g',  b.rel(p))

        //w.D(500, 400, 'b', [-100, 0], [0, -100], [100, 50])
        // w.D(500, 430).sep('w',  [-100, 0],  [0, -100],   [100, 50]  )
    ...
    GPC = function () {
        ...
        poly1 = M.poly([
            ...
            [61, 68],  [145, 122],  [186, 94],  [224, 135],  [204, 211],  [105, 200],  [141, 163],  [48, 139],  [74, 117]

        ])

        poly2 = M.poly([

            [131, 84],  [224, 110],  [174, 180], [120, 136], [60, 167],
        ...
        bt('difference', poly1.difference(poly2))
        bt('intersection', poly1.intersection(poly2))
        bt('union', poly1.union(poly2))
        bt('xor', poly1.xor(poly2))


        function bt(n, pol) {
            $.button(n, function(e){
                reset(); ctx.drawPolys(pol, 'g', 0, 150)
            }).A()}

    }
    POLYOPS = function(){W({g:0,w:0}).Y()
        p1 = M.poly( [-50, 50], [-50, -50],  [50, -50],   [50, 50],  [-45, 55]  )
        p2 = M.poly(  [-100, 0],  [-100, -100], [0, -100],  [0, 0],  [-98, 2] )
        w.S(300, 300).sep( p1 )
        w.S(300, 300).sep( p2 )
        $.in(1,function(){
            w.D(500, 200).sep(p1.union(p2))
            w.D(500, 200).sep(p1.difference(p2))
            w.D(500, 200).sep(p1.intersection(p2))
            w.D(500, 200).sep(p1.xor(p2))})
    ...
    UNION = function () {
        ...
        w.S(300, 100).sep({
            c: 'y',
            v: M.poly(w.S(300, 300, 'r', 50, 50))
                .union(
                M.poly(w.S(320, 320, 'b', 50, 50)))
        })
    }
}; elegant()
...
TERR = function () {W()
    ...
    circ = function (v, rad, prec) {
        ...
        ang = 2 * M.PI / prec
        _.t(prec, function (i) {
            pX = v.x + rad * M.cos(ang * i)
            pY = v.y + rad * M.sin(ang * i)
            arr.push(V(pX, pY))})
        return arr
    }
    ...
    explosion = function (x, y) {
        ...
        cir = M.poly( circ(V(x, y), 30, 5))

        bod = w.D(x,y).sep( circ(V(0,0),30,5)   )
    ...
    w.S(100,100).sep(t.bod.f().vs())



    dif = t.poly.difference(  M.poly( explosion(740, 395).cir ) )
...
//b.polygon = function () {var b=this; return M.poly( b.vs() )}
...
////////////////// WeilerAtherton  /////////////
    ...
    p.num = p.numPoints=function(){var p=this
       if (p.m_List.get(0))
       {return p.getNumPoints()}
    }

    p.vs = p.verts= function(){var p=this,vs=[]

        if(p.num()){
            _.t(p.getNumPoints(), function(i){
    ...
    p.num = p.numPoints=function(){var p=this
        if (p.m_List.get(0)){return p.getNumPoints()}
