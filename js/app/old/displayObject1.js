...
ob.accelerate=function(){return this.vX(this.ax||0,'+').vY(this.ay||0,'+')}
ob.jerk=function(){
...
ob.keyControls = function(num){
    ...
    if(args.P){
        $.kD('left', function(){ob.toL(num)})
        $.kD('right', function(){ob.toR(num)})}

    if(args.N){
        $.kD('up', function(){ob.toU(num)})
        $.kD('down',   function(){ob.toD(num)} )}
}
...
ob.bounce=function(n){  n=N(n)?n:0
    ...
    J.tick(
        function(){var x=ob.x, y=ob.y
            if(x > w - ob.W() - n  || x <(n ) ){ ob.vX('-') }
            if(y> h - ob.H() -n|| y<(n )){ ob.vY('-')  }
        })
    return st}
ob.hitByBullet = ob.isPointInMyRectBounds =  function(){var ob=this, g=G(arguments),
...
ob.makeMeAKiller =function(){
    ...
    return this}
ob.in= ob.within =ob.inStage =  function(){var ob=this, st = ob.st(),
    g=G(arguments),
    x= ob.x,
    y= ob.y,
    w= stg.W()-100,
